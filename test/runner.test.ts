...
test("runner exposes only the fifteen CLI-backed G0/G1 suites", async () => {
  assert.deepEqual(Object.keys(suiteIds), ["architecture-boundaries", "compose-smoke", "agent-adapters", "harness-policy", "open-source", "design-system", "spec-fuzz", "action-contract", "capability-contract", "graphify-policy", "spec-migrations", "package-matrix", "delivery-guardrails", "self-check", "supply-chain"]);
  const gates: Record<string, string> = { "design-system": "G1", "spec-fuzz": "G1", "action-contract": "G1", "capability-contract": "G1", "graphify-policy": "G1", "spec-migrations": "G1", "package-matrix": "G1", "delivery-guardrails": "G1", "self-check": "G1", "supply-chain": "G1" };
  ...
  assert.equal(JSON.parse(stdout).length, 15);
});

...
test("runner exposes only the ten CLI-backed G0/G1 suites", async () => {
  assert.deepEqual(Object.keys(suiteIds), ["architecture-boundaries", "compose-smoke", "agent-adapters", "harness-policy", "open-source", "design-system", "spec-fuzz", "action-contract", "capability-contract", "graphify-policy"]);
  const gates: Record<string, string> = { "design-system": "G1", "spec-fuzz": "G1", "action-contract": "G1", "capability-contract": "G1", "graphify-policy": "G1" };
  ...
  assert.equal(JSON.parse(stdout).length, 10);
});
